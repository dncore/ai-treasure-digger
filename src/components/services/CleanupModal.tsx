...
const CATEGORY_CONFIG: Record<CleanupCategory, { icon: string; label: string; color: string }> = {
  Safe: { icon: "\u{1F7E2}", label: "Rebuildable", color: "border-emerald-800/50 bg-emerald-950/20" },
  Warning: { icon: "\u{1F7E1}", label: "Not rebuildable", color: "border-amber-800/50 bg-amber-950/20" },
  Source: { icon: "\u{1F534}", label: "Source code", color: "border-red-800/50 bg-red-950/20" },
};
...
export function CleanupModal({ service, onClose }: CleanupModalProps) {
  ...
  async function loadTargets() {
    setLoading(true);
    try {
      const t = await getCleanupTargets(service.id);
      setTargets(t);
      const defaultSelected = new Set(
        t.filter((item) => item.category === "Safe").map((item) => item.path)
      );
      setSelectedPaths(defaultSelected);
    } catch (e) {
      ToastContext.addToast({ type: "error", title: "Failed to load cleanup targets", detail: String(e) });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleStartCleanup() {
    ...
    try {
      ...
      ToastContext.addToast({ type: "success", title: "Cleanup completed" });
      onClose();
    } catch (e) {
      ToastContext.addToast({ type: "error", title: "Cleanup failed", detail: String(e) });
    }
  }
  ...
  async function handleAbort() {
    try {
      await abortCleanup();
      ToastContext.addToast({ type: "warning", title: "Cleanup aborted" });
      onClose();
    } catch (e) {
  ...
  const totalSize = targets
    .filter((t) => selectedPaths.has(t.path))
    .reduce((acc, t) => acc + t.size, 0);

  const grouped = {
    Safe: targets.filter((t) => t.category === "Safe"),
    Warning: targets.filter((t) => t.category === "Warning"),
    Source: targets.filter((t) => t.category === "Source"),
  };
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold">Cleanup: {service.name}</h2>
          <button onClick={onClose} style={{ color: "var(--text-muted)" }}>&#x2715;</button>
        </div>

        {step === "select" && loading && (
          <div className="py-8 text-center" style={{ color: "var(--text-muted)" }}>Loading targets...</div>
        )}

        {step === "select" && !loading && targets.length === 0 && (
          <div className="py-8 text-center" style={{ color: "var(--text-muted)" }}>No cleanup targets found.</div>
        )}
          ...
          <div className="space-y-4 max-h-[60vh] overflow-auto">
            ...
            <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: "var(--border)" }}>
              <span className="text-sm" style={{ color: "var(--text-secondary)" }}>
                {selectedPaths.size} items, {formatBytes(totalSize)}
              </span>
              <button
              ...
              >
                {service.service_type === "WslInstance" ? "Next: Confirm" : "Start Cleanup"}
              </button>
            </div>
          </div>
        )}
        ...
        {step === "confirm-wsl" && (
          <div className="space-y-4">
            <div className="rounded-lg border border-red-800 bg-red-950/50 p-4">
              <h3 className="font-semibold text-red-300 mb-2">WSL Instance Warning</h3>
              <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
                Stopping a WSL instance will terminate all processes inside it. Unsaved data will be lost.
              </p>
            </div>
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" id="wsl-c1" className="accent-red-500" />
                I understand that all running processes will be terminated
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" id="wsl-c2" className="accent-red-500" />
                I understand that unsaved data will be lost and cannot be recovered
              </label>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between text-sm">
              <span style={{ color: "var(--text-secondary)" }}>
                {progress ? `Cleaning ${progress.current}/${progress.total}...` : "Starting cleanup..."}
              </span>
              {progress && (
